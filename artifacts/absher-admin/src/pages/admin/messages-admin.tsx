...
export default function MessagesAdmin() {
  ...
  if (isLoading) {
    return <div className="space-y-3">{[1,2,3,4].map(i => <div key={i} className="bg-card rounded-2xl border h-20 animate-pulse" />)}</div>;
  }

  return (
    <div className="space-y-5">
      {/* Filter tabs */}
      <div className="bg-card rounded-2xl border border-card-border p-2 flex gap-1 shadow-sm">
        {[
          ...
          <button
            ...
            className={`flex-1 py-2.5 px-4 rounded-xl text-sm font-medium transition-all ${filter === f.key ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground hover:bg-muted"}`}
          >
            {ar ? f.ar : f.en}
            {f.key === "unread" && (
              <span className="ms-2 bg-red-500 text-white text-xs rounded-full px-1.5 py-0.5">
        ...
        {!filtered?.length ? (
          ...
          filtered.map((msg) => {
            ...
            return (
              <div
                key={msg.id}
                className={`bg-card rounded-2xl border overflow-hidden shadow-sm transition-all ${!msg.read ? "border-primary/20" : "border-card-border"}`}
              >
                <div
                  className="flex items-start gap-4 px-6 py-4 cursor-pointer hover:bg-muted/30 transition-colors"
                  onClick={() => {
                    setExpanded(isExpanded ? null : msg.id);
                    if (!msg.read) markRead(msg.id);
                  }}
                >
                  <div className={`mt-1.5 w-2.5 h-2.5 rounded-full shrink-0 ${!msg.read ? "bg-primary" : "bg-muted"}`} />

                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 flex-wrap">
                      <span className="font-semibold text-foreground">{msg.name}</span>
                      {msg.subject && <span className="text-muted-foreground text-sm hidden md:block">— {msg.subject}</span>}
                      {!msg.read && (
                  ...
                  <div className="shrink-0 text-end">
                    ...
                    {msg.read
                      ? <CheckCircle className="h-4 w-4 text-green-400 mt-1 ms-auto" />
                      : <Eye className="h-4 w-4 text-primary mt-1 ms-auto" />
                    }
                  </div>
                </div>

                {isExpanded && (
                  <div className="border-t border-card-border px-6 py-5 bg-muted/20 space-y-4">
