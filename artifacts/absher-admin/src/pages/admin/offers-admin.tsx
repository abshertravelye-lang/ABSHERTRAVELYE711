    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl my-8">
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-xl font-bold">{ar ? "تفاصيل العرض" : "Offer Details"}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="md:col-span-2">
              <label className="flex items-center gap-3 cursor-pointer select-none">
                <div onClick={() => set("featured", !form.featured)} className={`w-12 h-6 rounded-full transition-colors ${form.featured ? "bg-primary" : "bg-slate-200"} relative`}>
                  <div className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${form.featured ? "start-6" : "start-0.5"}`} />
                </div>
                <span className="text-sm font-medium">{ar ? "عرض مميز" : "Featured Offer"}</span>
              </label>
            </div>
          </div>
        </div>
        <div className="flex justify-end gap-3 p-6 border-t bg-slate-50/50 rounded-b-2xl">
          <button onClick={onCancel} className="px-5 py-2.5 text-sm font-medium text-slate-600 hover:text-slate-800">
            {ar ? "إلغاء" : "Cancel"}
          </button>
