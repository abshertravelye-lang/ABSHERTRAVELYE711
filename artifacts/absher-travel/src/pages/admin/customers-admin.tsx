...
export default function CustomersAdmin() {
  ...
  const filtered = employees?.filter(e => {
    ...
    const matchSearch = !q || (e.firstName?.toLowerCase().includes(q) ?? false) || (e.lastName?.toLowerCase().includes(q) ?? false) || (e.email?.toLowerCase().includes(q) ?? false) || (e.phone?.includes(q) ?? false);
    return matchRole && matchSearch;
  });

  if (isLoading) return (
    <div className="space-y-3">{[1,2,3,4,5].map(i => <div key={i} className="bg-white rounded-2xl border h-16 animate-pulse" />)}</div>
  );

  return (
    <div className="space-y-5">
      {/* Search + filter */}
      <div className="bg-white rounded-2xl border border-slate-100 p-5 flex flex-wrap gap-4">
        <div className="flex-1 min-w-[200px]">
          <input
            placeholder={ar ? "بحث بالاسم أو البريد أو الهاتف..." : "Search by name, email or phone..."}
            className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20"
            value={search}
        ...
        <div className="flex gap-2 flex-wrap">
          {["all", "customer", "agent", "admin", "super_admin"].map(r => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${roleFilter === r ? "bg-primary text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={role} className="bg-white rounded-2xl border border-slate-100 p-4 flex items-center gap-3">
            ...
            <div>
              <div className="text-2xl font-bold text-slate-800">{employees?.filter(e => e.role === role).length ?? 0}</div>
              <div className="text-xs text-slate-500">{ar ? ROLE_LABELS[role]?.ar : ROLE_LABELS[role]?.en}</div>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>
                <th className="text-start rtl:text-end px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider">{ar ? "المستخدم" : "User"}</th>
                <th className="text-start rtl:text-end px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider">{ar ? "التواصل" : "Contact"}</th>
                <th className="text-start rtl:text-end px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider">{ar ? "الدور" : "Role"}</th>
                <th className="text-start rtl:text-end px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider">{ar ? "الحالة" : "Status"}</th>
                <th className="text-start rtl:text-end px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider">{ar ? "تاريخ الانضمام" : "Joined"}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {!filtered?.length ? (
                <tr><td colSpan={5} className="text-center py-16 text-slate-400">{ar ? "لا يوجد مستخدمون" : "No users found"}</td></tr>
              ) : (
                filtered.map(emp => (
                  <tr key={emp.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-slate-400 truncate max-w-[160px]">{emp.email ?? emp.phone ?? "—"}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${ROLE_LABELS[emp.role]?.color ?? "bg-slate-100 text-slate-600"}`}>
